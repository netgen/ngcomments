(function($) {
	var ngComments = function(options) {
		...
		editCommentCancel = function(w, trigger) {
			var comment_container = trigger.closest('.ezcom-view-comment');
			comment_container.children('.ezcom-view-comment-item').show();
			comment_container.children('.ezcom-comment-tool').show();
			comment_container.children('[name="CommentEdit"]').remove();
			return false;
		},

		editCommentSubmit = function(w, trigger, missing_input_message) {
			var comment_container = trigger.closest('.ezcom-view-comment');
			var edit_comment_container = comment_container.children('[name="CommentEdit"]');
		...
		addOwnerMarkup = function(w, comment) {
			...
			if (comment.hasClass(opt.p.uoid)) {
				comment.append(opt.p.owner_controls);
			}
		},
